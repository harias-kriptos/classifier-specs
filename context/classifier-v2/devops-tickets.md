...
| `crown-enterprise-barrier` | ❌ falta | KT-17013 AC01 |
| `crown-validation-handler` | ❌ falta | KT-17014 AC01 |
| `crown-validation-confirm` | ❌ falta | KT-17015 AC01 |
...
| `validated_crown_jewels` | 📋 **NUEVO** | (a crear) | crown-validation-confirm (N4 W), gse-cycle-init (N5 R) | Encryption + public-access block + EventBridge sobre `manifest.json` → SQS FIFO `gse-validated-cycle-queue.fifo`. Lifecycle 30 días. |
...
|---|---|---|---|---|---|
| `crown-enterprise-barrier-pipe` | 📋 **NUEVO** | DDB Stream `crown-validation-state` | `eventName IN ["MODIFY","INSERT"] AND NewImage.SK begins_with "PHASE1_STATION#"` | `lambda-crown-enterprise-barrier` (N2) | Sí, propio + alarma |
...
| `POST /v2/validation/confirm` | 📋 **NUEVO** | lambda-crown-validation-confirm (N4) | API key (+ recomendado WAF para tenant validation upstream en Plataforma Web) | Misma HTTP API que `/v2/tree/init` (agregar route, no nueva API). |
...
| `lambda-crown-enterprise-barrier` (N2) | 📋 **NUEVO** | 📋 **NUEVO** | 256 MB | 30 s |
| `lambda-crown-validation-handler` (N3) | 📋 **NUEVO** | 📋 **NUEVO** | 512 MB | 60 s |
| `lambda-crown-validation-confirm` (N4) | 📋 **NUEVO** | 📋 **NUEVO** | 1024 MB | 300 s |
...
| N2 crown-enterprise-barrier | `ddb:UpdateItem/GetItem` (`crown-validation-state`), `sns:Publish` (notify channel — stub) |
| N3 crown-validation-handler | `es:ESHttpPost/Put`, `ddb:UpdateItem/GetItem` (`crown-validation-state`) |
| N4 crown-validation-confirm | `es:ESHttpPost` (scroll), `s3:PutObject` (`validated_crown_jewels/*`), `ddb:UpdateItem/GetItem` (`crown-validation-state`) |
...
|---|---|---|---|---|
| `phase1-ready-for-validation` | 📋 **NUEVO** | N2 crown-enterprise-barrier | Plataforma Web | **Stub temporal** — canal definitivo lo define Plataforma Web (puede ser GraphQL subscription, webhook, email). Crear como SNS topic con sub vacío para no bloquear backend. |
...
| Resolver / invoker que llama a `lambda-crown-validation-handler` (N3) | 📋 **NUEVO** | Plataforma Web |
...
| [KT-17013](https://kriptosteam.atlassian.net/browse/KT-17013) | `lambda-crown-enterprise-barrier` (N2) + EventBridge Pipe sobre DDB Stream | Plataforma Web (consume notify) |
| [KT-17014](https://kriptosteam.atlassian.net/browse/KT-17014) | `lambda-crown-validation-handler` (N3) + invocación desde GraphQL Plataforma Web | Plataforma Web |
| [KT-17015](https://kriptosteam.atlassian.net/browse/KT-17015) | `lambda-crown-validation-confirm` (N4) + API GW route `POST /v2/validation/confirm` + bucket `validated_crown_jewels` + SQS FIFO `gse-validated-cycle-queue.fifo` | KT-17018 (Fase 2) |
...
8. [KT-17015](https://kriptosteam.atlassian.net/browse/KT-17015) (crown-validation-confirm + bucket validated + SQS FIFO + API GW).
9. [KT-17014](https://kriptosteam.atlassian.net/browse/KT-17014) (crown-validation-handler — depende del schema GraphQL de Plataforma Web).
